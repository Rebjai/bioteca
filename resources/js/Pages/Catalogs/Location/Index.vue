...
import LocationForm from '@/Components/forms/Location/LocationForm.vue';
                    ...
                    <div class="md:mx-5 p-6 bg-zinc-200 border-b overflow-x-auto border-gray-200 rounded-lg">
                        <table class="min-w-full">
                            <thead class="border-b-2 border-zinc-500 pb-8">
                                ...
                                <th class="px-5">Municipio</th>
                                <th class="px-5">Coordenadas</th>
                                <!-- shows address, map with coordinates -->
                                <th>Acciones</th>
                            </thead>
                            <tbody v-if="locations?.data?.length">
                                <tr v-for="location in locations.data" :key="location.id">
                                    <td class="px-3">{{location.id}}</td>
                                    <!-- <td class="px-3">{{location.species.scientific_name}}</td> -->
                                    <td class="px-3">{{location.name}}</td>
                                    <td class="px-3">{{location.municipality.name}}</td>
                                    <td class="px-3">{{location.latitude}}</td>
                                    <td class="flex flex-col  text-center items-center justify-center">
                                        <!-- <Link :href="route('location.show', location.id)" class="m-1 md:m-2 py-2 px-3 bg-blue-00 rounded-lg text-base block">
                                        <button>
                                        ...
                                        <Link :href="route('location.edit', location.id)" class="m-1 md:m-2 py-2 px-3 bg-blue-00 rounded-lg text-base block">
                                        <button>
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                                        ...
                                        <Link :href="route('location.destroy', location.id)" class="m-1 md:m-2 py-2 px-3 bg-blue-00 rounded-lg text-base block">
                                        <button>
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
