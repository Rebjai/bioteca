...
import StateForm from '@/Components/forms/State/StateForm.vue';
...
const states = props.states? props.states : []
function addUser(){
...
<template>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded flex flex-col justify-center items-center">
                    ...
                    <div class="md:mx-5 p-6 bg-zinc-200 border-b overflow-x-auto border-gray-200 rounded-lg">
                        <table class="min-w-full">
                            ...
                            <tbody v-if="states?.data?.length">
                                <tr v-for="state in states.data" :key="state.id">
                                    <td class="px-3">{{state.id}}</td>
                                    <!-- <td class="px-3">{{state.species.scientific_name}}</td> -->
                                    <td class="px-3">{{state.name}}</td>
                                    <td class="flex flex-col  text-center items-center justify-center">
                                        <!-- <Link :href="route('state.show', state.id)" class="m-1 md:m-2 py-2 px-3 bg-blue-00 rounded-lg text-base block">
                                        <button>
                                        ...
                                        <Link :href="route('state.edit', state.id)" class="m-1 md:m-2 py-2 px-3 bg-blue-00 rounded-lg text-base block">
                                        <button>
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                                        ...
                                        <Link :href="route('state.destroy', state.id)" class="m-1 md:m-2 py-2 px-3 bg-blue-00 rounded-lg text-base block">
                                        <button>
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                        ...
                        <pagination :links="states.links"></pagination>
                    </div>
                </div>
            </div>
        </div>
    ...
    <BioModal :isShowing="showModal" @close="showModal = false" title="Datos Generales">
        <div class="bg-zinc-100 rounded-lg py-5 max-h-full">
            <h1 class="text-center text-2xl mb-5 font-bold">Datos del Estado</h1>
            <StateForm />
        </div>
