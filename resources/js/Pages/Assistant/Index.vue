...
const props = defineProps({assistants:{}})
const showModal = ref(false)
const assistants = props.assistants? props.assistants : []
function addUser(){
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded flex flex-col justify-center items-center">
                    ...
                    <div class="md:mx-5 p-6 bg-zinc-200 border-b overflow-x-auto border-gray-200 rounded-lg">
                        <table class="min-w-full">
                            ...
                            <tbody v-if="assistants?.data?.length">
                                <tr v-for="assistant in assistants.data" :key="assistant.id">
                                    <td class="px-3">{{assistant.id}}</td>
                                    <!-- <td class="px-3">{{assistant.species.scientific_name}}</td> -->
                                    <td class="px-3">{{assistant.name}}</td>
                                    <td class="px-3">{{assistant.first_surname}}</td>
                                    <td class="px-3">{{assistant.second_surname}}</td>
                                    <td class="flex flex-col  text-center items-center justify-center">
                                        <!-- <Link :href="route('assistant.show', assistant.id)" class="m-1 md:m-2 py-2 px-3 bg-blue-00 rounded-lg text-base block">
                                        <button>
                                        ...
                                        <Link :href="route('assistant.edit', assistant.id)" class="m-1 md:m-2 py-2 px-3 bg-blue-00 rounded-lg text-base block">
                                        <button>
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                                        ...
                                        <Link :href="route('assistant.destroy', assistant.id)" class="m-1 md:m-2 py-2 px-3 bg-blue-00 rounded-lg text-base block">
                                        <button>
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                        ...
                        <pagination :links="assistants.links"></pagination>
                    </div>
                </div>
            </div>
        </div>
